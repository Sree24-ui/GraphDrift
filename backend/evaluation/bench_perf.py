...
# 10k pool omitted: building and timing it costs more than the curve's shape
# is worth once 500-5,000 is fitted.
...
def patch_results_md(report: dict) -> None:
    """Replace only this script's own block.

    It used to own everything from "## Performance benchmarks" to the end
    marker, which swallowed the hand-written pre-fix comparison tables on every
    run. Now it owns the text between its two markers, and refuses to run if
    they are missing rather than guessing where to write.
    """
    start_marker = "<!-- perf-bench -->"
    end_marker = "<!-- /perf-bench -->"
    text = RESULTS_MD.read_text()
    if start_marker not in text or end_marker not in text:
        raise SystemExit(
            f"{RESULTS_MD} is missing the {start_marker} ... {end_marker} block; "
            "restore it before re-running this benchmark."
        )
    block = format_report(report).rstrip() + "\n"
    start = text.find(start_marker) + len(start_marker)
    end = text.find(end_marker)
    RESULTS_MD.write_text(text[:start] + "\n" + block + text[end:])
...
def format_report(report: dict) -> str:
    ...
    if curve:
        largest = curve[-1]
        # Name whichever phase actually dominates the largest measured cycle,
        # rather than restating whichever one did when this text was written.
        phases = {
            "Layer 2 (graph + Louvain + hub concentration)": "layer2",
            "persist (explanation assembly + SQLite writes)": "persist",
            "the peripheral cascade": "peripheral",
            "feature extraction": "features",
        }
        totals = largest["summary"]["total"]["mean"]
        label, key = max(
            phases.items(), key=lambda kv: largest["summary"].get(kv[1], {}).get("mean", 0)
        )
        share = largest["summary"].get(key, {}).get("mean", 0) / totals if totals else 0
        dominant = (
            f" At the largest scale the biggest slice is {label}, "
            f"{share:.0%} of the cycle."
        )
        mean_seconds = largest["summary"]["total"]["mean"]
        share = mean_seconds / DETECTION_CYCLE_INTERVAL_SECONDS
        verdict = "fits inside" if share < 1 else "exceeds"
        headroom = (
            f" At {largest['target']:,} accounts the mean cycle is "
            f"{mean_seconds:.1f}s, which {verdict} the "
            f"{DETECTION_CYCLE_INTERVAL_SECONDS:.0f}s live interval ({share:.0%} of it)."
        )
    lines += [
        "",
        f"Log-log slope of mean cycle time vs 15m-active accounts: **{slope_txt}** "
        f"({trend}). Slope 1 is linear. A 10,000-account target was not timed."
        f"{dominant}{headroom}",
        "",
        ...
        f"The {e2e['cycle_interval_seconds']:.0f}s interval, not SQLite, dominates "
        f"analyst-visible delay at this scale; the scalability table above shows where "
        f"cycle compute itself starts to matter.",
        "",
        "### Honest read",
        "",
        report["honest"],
